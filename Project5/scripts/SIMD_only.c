...
// dense GEMM kernel replaced with AVX2 version
...
#if defined(__FMA__)
                c8 = _mm256_fmadd_ps(a8, b8, c8);  // c += a*b
#else
...
// CSR SpMM kernel replaced with AVX2 version
...
int main(int argc, char **argv) {
    ...
    printf("Exp1 Baseline (scalar, single-thread)\n");
    printf("m=%d k=%d n=%d, density=%.4f, repeats=%d\n", m, k, n, density, repeats);
