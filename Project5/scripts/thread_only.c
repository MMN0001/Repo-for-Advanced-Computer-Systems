...
//thread-only GEMM parallelize over output rows i
static void dense_gemm_thread_only(const DenseMatrix *A,
                                  const DenseMatrix *B,
                                  DenseMatrix *C) {
    ...
    for (int i = 0; i < M; ++i) {
        for (int k = 0; k < K; ++k) {
            float a = dense_get(A, i, k);
            for (int j = 0; j < N; ++j) {
                dense_add(C, i, j, a * dense_get(B, k, j));
            }
        }
    }
}

//thread-only SpMM parallelize over CSR rows i
static void csr_spmm_thread_only(const CSRMatrix *A,
                                const DenseMatrix *B,
                                DenseMatrix *C) {
    assert(A->cols == B->rows);
    assert(C->rows == A->rows && C->cols == B->cols);
    ...
    for (int i = 0; i < M; ++i) {
        for (int idx = A->row_ptr[i]; idx < A->row_ptr[i + 1]; ++idx) {
            int j = A->col_idx[idx];
            float a = A->values[idx];
            for (int col = 0; col < N; ++col) {
                dense_add(C, i, col, a * dense_get(B, j, col));
            }
        }
    }
}
...
int main(int argc, char **argv) {
    ...
    // ./a.out m k n density repeats threads
    int threads = 0;
    ...
    printf("Exp1 Thread-only (scalar + OpenMP)\n");
    printf("m=%d k=%d n=%d, density=%.4f, repeats=%d, threads=%d\n",
           m, k, n, density, repeats, (threads > 0 ? threads : omp_get_max_threads()));
    ...
    // correctness check
    {
        ...
        dense_zero(&C_my);
        dense_gemm_thread_only(&A_test, &B, &C_my);
...

    {
        ...
        dense_zero(&C_spmm);
        csr_spmm_thread_only(&A_csr, &B, &C_spmm);
    ...
    for (int t = 0; t < repeats; ++t) {
        ...
        double t0 = now_sec();
        csr_spmm_thread_only(&A_csr, &B, &C);
        double t1 = now_sec();
